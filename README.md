# My Solution to ZK Hack Mini Puzzle2

This is my solution to ZK Hack Mini Puzzle 2 as found at:

https://www.zkhack.dev/puzzleM2.html

## Description of my solution:

At first, I merged Kobi's PR3 into the initial Repo, which was a truly helpful hint towards the solution.

Then I use the Lagrange interpolation methods which is already a part of the Winterfall math library to create a degree 3 polynomial out of 3 arbitrary values of the base domain in the degree reducing projection, provided by: `math::polynom::{interpolate, eval}`.

Additionally, I use the `rand` trait to generate random values as depictable interpolation points. The folding domain is of dimension 32, giving 35960 possible combinations to pick evaluation points. While the probability is quite high to pick valid points, I still found randomizing useful to quickly find a valid selection.

Then, I select 4 random indices for domain points:

```rust
    let mut rng = rand::thread_rng();
    let selection = vec![rng.gen_range(0..31),rng.gen_range(0..31), rng.gen_range(0..31), rng.gen_range(0..31)];
```

I print the 4 selected points, and define the interposlation domains `xs` and `ys` for the polynomial:

```rust
    println!("selection: {:?}", selection);
    let xs = &vec![domain[selection[0]], domain[selection[1]], domain[selection[2]], domain[selection[3]]];
    let ys = &vec![result[selection[0]], result[selection[1]], result[selection[2]], result[selection[3]]];
```

Finally, I compute the polynomial with Lagrange interpolation and re-evaluate the points with the derived polynomial:

```rust
    let poly = interpolate(xs, ys, false);
    result = domain.iter().map(|value| eval(&poly, *value)).collect();
```

Now I compile the code and re-execute it, until my interpolation points hit some correct query points (probability to hit valid points is probably around 1/8).

I found this to be a valid selection of indices depicting points in the evaluation domain:

```rust
let selection = vec![14, 8, 15, 0];
```

With this selection, all I have to do is replacing the HINT comment in `/fri/src/folding/mod.rs` by:
...
```
cargo run --release -- -q 1 fib -n 32
```

yields to a correct verification of the proof.
...
Next, we need to define _algebraic intermediate representation_ (AIR) for our computation. This process is usually called _arithmetization_. We do this by implementing the `Air` trait. At the high level, the code below does three things:
...
If you are interested in learning how STARKs work under the hood, here are a few links to get you started. From the standpoint of this library, _arithmetization_ is by far the most important concept to understand.
